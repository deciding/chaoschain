...
class ChaosAgent:
    ...
    def __init__(
        ...
        rprint(
            f"[green]🌐 Connected to {self.network} (Chain ID: {self.chain_id})[/green]"
        )
    ...
    def _load_contract_addresses(self):
        ...
        contract_addresses = {
            # ═══ MAINNET: Ethereum, Base, Polygon, Arbitrum, Celo, Gnosis, Scroll, Taiko, Monad, BSC ═══
            NetworkConfig.ETHEREUM_MAINNET: _mainnet(
            ...
            NetworkConfig.BASE_SEPOLIA: _testnet(
            ...
            NetworkConfig.OPTIMISM_SEPOLIA: _testnet(
                usdc="0x5fd84259d66Cd46123540766Be93DFE6D43130D7", validation=ZERO
            ),
            ...
            NetworkConfig.ZEROG_TESTNET: {
                ...
                "usdc_token": "0x0000000000000000000000000000000000000000",  # 0G uses native A0GI
        ...
        if not network_contracts:
            raise ConfigurationError(
                f"No deployed contracts configured for network: {self.network}"
            )
    ...
    def _load_contracts(self):
        ...
        try:
            ...
            self.identity_registry = self.w3.eth.contract(
                address=self.contract_addresses.identity_registry, abi=identity_abi
            )

            self.reputation_registry = self.w3.eth.contract(
                address=self.contract_addresses.reputation_registry, abi=reputation_abi
            )

            self.validation_registry = self.w3.eth.contract(
                address=self.contract_addresses.validation_registry, abi=validation_abi
            )
            ...
            # Core Functions (Feb 2026 ABI)
            {
                "inputs": [
                    {"name": "agentId", "type": "uint256"},
                    {
                        "name": "value",
                        "type": "int128",
                    },  # CHANGED: uint8 score -> int128 value
                    {
                        "name": "valueDecimals",
                        "type": "uint8",
            ...
            # Read Functions (Feb 2026 ABI)
            {
                ...
                "outputs": [
                    {"name": "count", "type": "uint64"},
                    {
                        "name": "summaryValue",
                        "type": "int128",
                    },  # CHANGED: uint8 averageScore -> int128 summaryValue
                    {
            ...
            },
            {
                ...
                "outputs": [
                    ...
                    {"name": "feedbackIndexes", "type": "uint64[]"},
                    {
                        "name": "values",
                        "type": "int128[]",
                    },  # CHANGED: uint8[] scores -> int128[] values
                    {
                        "name": "valueDecimals",
                        "type": "uint8[]",
                    },  # NEW: decimal precisions
    ...
    def register_agent(
        ...
        """
        rprint(f"\n{'=' * 60}")
        rprint(f"🔥🔥🔥 LOCAL CHAOSCHAIN SDK IS BEING USED! 🔥🔥🔥")
        rprint(f"🔥🔥🔥 agent_name={self.agent_name} 🔥🔥🔥")
        rprint(f"{'=' * 60}\n")
        rprint(
            f"[yellow]🔧 Registering agent: {self.agent_name} ({self.agent_domain})[/yellow]"
        )
        ...
        try:
            ...
            if balance > 0:
                rprint(
                    f"[blue]🔍 Wallet owns {balance} agent NFT(s), checking...[/blue]"
                )

                # Use tokenOfOwnerByIndex to get the first agent ID owned by this wallet
                try:
                    existing_agent_id = (
                        self.identity_registry.functions.tokenOfOwnerByIndex(
                            self.address, 0
                        ).call()
                    )
                    self.agent_id = existing_agent_id
                    rprint(
                        f"[green]✅ Agent already registered with ID: {self.agent_id}[/green]"
                    )
                    return self.agent_id, "already_registered"
                except Exception as enum_error:
                    # If tokenOfOwnerByIndex not available, try iterating through recent tokens
                    rprint(
                        f"[yellow]⚠️  Enumerable not available, checking recent tokens...[/yellow]"
                    )
                    try:
                        total_supply = (
                            self.identity_registry.functions.totalSupply().call()
                        )
                        # Check last 100 tokens for ownership
                        for potential_id in range(
                            total_supply, max(0, total_supply - 100), -1
                        ):
                            try:
                                owner = self.identity_registry.functions.ownerOf(
                                    potential_id
                                ).call()
        ...
        try:
            ...
            if metadata:
                ...
                rprint(
                    f"[blue]📋 Registering with {len(metadata_entries)} metadata entries[/blue]"
                )
                contract_call = self.identity_registry.functions[
                    "register(string,(string,bytes)[])"
                ](token_uri, metadata_entries)
            else:
                # Use simple register(tokenURI) function
                contract_call = self.identity_registry.functions["register(string)"](
                    token_uri
                )
            ...
            rprint(
                f"[yellow]⛽ Gas estimate: {gas_estimate}, using limit: {gas_limit}[/yellow]"
            )
            ...
            if receipt.status == 1:
                ...
                try:
                    ...
                    if logs:
                        ...
                        rprint(
                            f"[green]✅ Agent registered successfully with ID: {self.agent_id}[/green]"
                        )
                        rprint(
                            f"[blue]📋 View on explorer: Transaction {tx_hash.hex()[:10]}...[/blue]"
                        )
                        return self.agent_id, tx_hash.hex()
                except Exception as log_error:
                    rprint(
                        f"[yellow]⚠️  Could not parse event logs: {log_error}[/yellow]"
                    )
                    ...
                    try:
                        ...
                        rprint(
                            f"[green]✅ Agent registered with ID: {self.agent_id}[/green]"
                        )
                        return self.agent_id, tx_hash.hex()
                    except:
                        ...
                        try:
                            ...
                            for potential_id in range(
                                total_supply, max(0, total_supply - 10), -1
                            ):
                                try:
                                    owner = self.identity_registry.functions.ownerOf(
                                        potential_id
                    ...
                    raise AgentRegistrationError(
                        "Registration succeeded but could not determine agent ID"
                    )
            else:
            ...
            if "insufficient funds" in error_msg.lower():
                rprint(
                    f"[yellow]💰 Insufficient ETH for gas fees in wallet: {self.address}[/yellow]"
                )
                rprint(
                    f"[blue]Please fund this wallet using Base Sepolia faucet:[/blue]"
                )
                rprint(
                    f"[blue]https://www.coinbase.com/faucets/base-ethereum-sepolia-faucet[/blue]"
                )

            raise AgentRegistrationError(
                f"Failed to register {self.agent_domain}: {error_msg}"
            )
    ...
    def get_agent_id(self, use_cache: bool = True) -> Optional[AgentID]:
        ...
        try:
            ...
            try:
                ...
                agent_id = self.identity_registry.functions.tokenOfOwnerByIndex(
                    self.address, 0
                ).call()
    ...
    def get_reputation_score(self, agent_id: Optional[int] = None) -> float:
        ...
        try:
            ...
            reputation_registry_address = (
                self.registry.functions.getReputationRegistry().call()
            )
            ...
            reputation_registry = self.w3.eth.contract(
                address=reputation_registry_address, abi=reputation_abi
            )
            ...
            rprint(
                f"[yellow]⚠️  Reputation fetching not fully implemented (agent_id: {agent_id})[/yellow]"
            )
    ...
    def set_agent_metadata(self, key: str, value: bytes) -> TransactionHash:
        ...
        if not self.agent_id:
            raise AgentRegistrationError(
                "Agent must be registered before setting metadata"
            )

        try:
            rprint(
                f"[yellow]📝 Setting metadata '{key}' for agent #{self.agent_id}[/yellow]"
            )
    ...
    def get_agent_metadata(self, key: str, agent_id: Optional[int] = None) -> bytes:
        ...
        if target_agent_id is None:
            raise AgentRegistrationError(
                "Agent ID required (either register or provide agent_id parameter)"
            )
        ...
        try:
            ...
            rprint(
                f"[green]✅ Retrieved metadata '{key}' for agent #{target_agent_id}[/green]"
            )
            ...
            error_msg = str(e)
            rprint(
                f"[yellow]⚠️  Could not retrieve metadata '{key}': {error_msg}[/yellow]"
            )
            raise ContractError(
                f"Failed to get metadata '{key}' for agent #{target_agent_id}: {error_msg}"
            )
    ...
    def request_validation(
        ...
        try:
            if not self.agent_id:
                raise ContractError(
                    "Agent must be registered before requesting validation"
                )
            ...
            rprint(
                f"[yellow]📋 Requesting validation from {validator_address[:10]}...[/yellow]"
            )
            ...
            contract_call = self.validation_registry.functions.validationRequest(
                validator_address, self.agent_id, request_uri, request_hash_bytes
            )
            ...
            if receipt.status == 1:
                rprint(
                    f"[green]✅ Validation request submitted: {tx_hash.hex()[:10]}...[/green]"
                )
                return tx_hash.hex()
            else:
    ...
    def submit_validation_response(
        ...
        try:
            ...
            if tag:
                ...
                tag_bytes = tag_encoded + b"\x00" * (
                    32 - len(tag_encoded)
                )  # Pad with zeros

            rprint(
                f"[yellow]✍️  Submitting validation response: {response}/100[/yellow]"
            )
            ...
            if receipt.status == 1:
                rprint(
                    f"[green]✅ Validation response submitted: {tx_hash.hex()[:10]}...[/green]"
                )
                return tx_hash.hex()
            else:
    ...
    def generate_feedback_authorization(
        self, agent_id: AgentID, client_address: str, index_limit: int, expiry: int
    ) -> bytes:
        """
    ...
    def give_feedback(
        ...
        try:
            ...
            rprint(
                f"[yellow]💬 Submitting feedback: {score}/100 for agent #{agent_id}[/yellow]"
            )
    ...
    def create_feedback_with_payment(
        ...
        **additional_fields,
        ...
        if payment_proof:
            ...
            to_address = (
                payment_proof.to_agent
                if hasattr(payment_proof, "to_agent")
                else "unknown"
            )
            if hasattr(payment_proof, "receipt_data") and isinstance(
                payment_proof.receipt_data, dict
            ):
            ...
            if hasattr(payment_proof, "network"):
                ...
                elif hasattr(payment_proof.network, "value") and hasattr(
                    payment_proof.network.value, "chain_id"
                ):
            ...
            rprint(
                f"[cyan]💳 Including payment proof: {payment_proof.transaction_hash[:10]}...[/cyan]"
            )
        ...
        rprint(
            f"[yellow]📝 Creating feedback JSON ({len(feedback_json)} bytes)[/yellow]"
        )
        ...
        try:
            ...
            storage_result = self._feedback_storage.put(
                feedback_json.encode("utf-8"), mime="application/json"
            )
            ...
            file_hash = (
                "0x" + hashlib.sha3_256(feedback_json.encode("utf-8")).hexdigest()
            )
    ...
    def get_reputation(
        ...
        try:
            ...
            for client in clients:
                ...
                last_index = reputation_registry.functions.getLastIndex(
                    agent_id, client
                ).call()
                ...
                for idx in range(last_index):
                    try:
                        ...
                        (
                            ...
                            feedback_tag2,
                            is_revoked,
                        ) = reputation_registry.functions.readFeedback(
                            agent_id, client, idx
                        ).call()
                            ...
                            all_feedback.append(
                                {
                                    ...
                                    "score": int(
                                        effective_score
                                    ),  # Keep backward compatible 'score' key
                                    "value": value,
                                    "valueDecimals": value_decimals,
                                    "tag1": feedback_tag1.hex()
                                    if isinstance(feedback_tag1, bytes)
                                    else str(feedback_tag1),
                                    "tag2": feedback_tag2.hex()
                                    if isinstance(feedback_tag2, bytes)
                                    else str(feedback_tag2),
                                    "index": idx,
                                }
                            )
                    except Exception as e:
    ...
    def get_reputation_summary(
        ...
        try:
            ...
            if client_addresses:
                clients = [
                    self.w3.to_checksum_address(addr) for addr in client_addresses
                ]
            ...
            count, avg_score = reputation_registry.functions.getSummary(
                agent_id, clients, tag1_bytes, tag2_bytes
            ).call()

            rprint(
                f"[green]✓[/green] Reputation summary: {count} entries, avg {avg_score}/100"
            )

            return {"count": count, "averageScore": avg_score}

        except Exception as e:
            raise ContractError(f"Failed to get reputation summary: {str(e)}")

    def submit_score_vector(
        self, studio_address: str, data_hash: bytes, score_vector: List[int]
    ) -> TransactionHash:
        """
        ...
        try:
            ...
            studio_proxy = self.w3.eth.contract(
                address=studio_address, abi=studio_proxy_abi
            )
            ...
            score_bytes = encode(
                ["uint8", "uint8", "uint8", "uint8", "uint8"], scores_padded
            )

            rprint(
                f"[cyan]📊 Submitting score vector to Studio {studio_address[:10]}...[/cyan]"
            )
            ...
            raw_transaction = getattr(
                signed_tx, "raw_transaction", getattr(signed_tx, "rawTransaction", None)
            )
            tx_hash = self.w3.eth.send_raw_transaction(raw_transaction)
    ...
    def submit_score_vector_for_worker(
        ...
        try:
            ...
            rprint(
                f"[cyan]📊 Submitting per-worker score vector to Studio {studio_address[:10]}...[/cyan]"
            )
            ...
            tx = studio_proxy.functions.submitScoreVectorForWorker(
                data_hash, worker_address, score_bytes
            ).build_transaction(
                {
                    "from": account.address,
                    "nonce": self.w3.eth.get_transaction_count(account.address),
            ...
            tx_hash_hex = tx_hash.hex()
            rprint(
                f"[green]✅ Per-worker score vector submitted for {worker_address[:10]}...: {tx_hash_hex[:10]}...[/green]"
            )
    ...
    def _generate_feedback_auth(self, agent_id: int, rewards_distributor: str) -> bytes:
        """
        ...
        try:
            ...
            signed_message = self.w3.eth.account.sign_message(
                signable_message, private_key=account.key
            )
