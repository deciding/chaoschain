...
class PaymentManager:
    ...
    def __init__(self, network: NetworkConfig, wallet_manager: WalletManager):
        ...
        self.treasury_address = (
            "0x20E7B2A2c8969725b88Dd3EF3a11Bc3353C83F70"  # ChaosChain treasury
        )
    ...
    def create_x402_payment_request(
        ...
        rprint(
            f"[blue]💰 Creating x402 payment request: {from_agent} → {to_agent} ({amount} {currency})[/blue]"
        )
    ...
    def execute_x402_payment(self, payment_request: Dict[str, Any]) -> PaymentProof:
        ...
        try:
            ...
            fee_tx_hash = self._transfer_usdc(
                from_agent, self.treasury_address, protocol_fee
            )
            ...
            payment_tx_hash = self._transfer_usdc(
                from_agent, self.wallet_manager.get_wallet_address(to_agent), net_amount
            )
    ...
    def _process_stripe_payment(
        self, amount: float, currency: str, payment_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not stripe_secret_key:
            ...
            rprint(
                f"[yellow]⚠️  STRIPE_SECRET_KEY not found - running in demo mode[/yellow]"
            )
            return self._create_demo_stripe_response(
                payment_id, amount, currency, payment_data
            )
        ...
        try:
            ...
            payment_intent = stripe.PaymentIntent.create(
                ...
                metadata={"chaoschain_payment_id": payment_id, "agent_payment": "true"},
            )
            ...
            rprint(
                f"[green]✅ Stripe Payment Intent created (REAL API): ${amount} {currency}[/green]"
            )
            ...
            rprint(
                f"[red]❌ Stripe library not installed. Install with: pip install stripe[/red]"
            )
            return self._create_demo_stripe_response(
                payment_id, amount, currency, payment_data
            )
    ...
    def _create_demo_stripe_response(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        rprint(
            f"[yellow]⚠️  Stripe demo mode (no API key): ${amount} {currency}[/yellow]"
        )
    ...
    def _process_google_pay_payment(
        ...
        if not google_pay_merchant_id:
            # Fallback to demo mode with clear indication
            rprint(
                f"[yellow]⚠️  GOOGLE_PAY_MERCHANT_ID not found - running in demo mode[/yellow]"
            )
            return self._create_demo_google_pay_response(
                payment_id, amount, currency, payment_data
            )
        ...
        try:
            ...
            if google_pay_gateway.lower() == "stripe":
                return self._process_google_pay_via_stripe(
                    payment_id, amount, currency, payment_token, payment_data
                )
            else:
                # For other gateways, implement similar logic
                return self._process_google_pay_generic(
                    payment_id,
                    amount,
                    currency,
                    payment_token,
    ...
    def _process_google_pay_via_stripe(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        payment_method = stripe.PaymentMethod.create(
            ...
            card={
                "token": payment_token[
                    "signedMessage"
                ]  # This would be processed differently in real implementation
            },
        )
        ...
        rprint(
            f"[green]✅ Google Pay processed via Stripe (REAL API): ${amount} {currency}[/green]"
        )
        rprint(f"   Payment Intent: {payment_intent.id}")
        rprint(f"   Status: {payment_intent.status}")
    ...
    def _process_google_pay_generic(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        rprint(
            f"[green]✅ Google Pay token validated (REAL): ${amount} {currency}[/green]"
        )
    ...
    def _create_demo_google_pay_response(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        rprint(
            f"[yellow]⚠️  Google Pay demo mode (no merchant ID): ${amount} {currency}[/yellow]"
        )
    ...
    def _process_apple_pay_payment(
        self, amount: float, currency: str, payment_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not apple_pay_merchant_id:
            # Fallback to demo mode with clear indication
            rprint(
                f"[yellow]⚠️  APPLE_PAY_MERCHANT_ID not found - running in demo mode[/yellow]"
            )
            return self._create_demo_apple_pay_response(
                payment_id, amount, currency, payment_data
            )
        ...
        try:
            ...
            if apple_pay_gateway.lower() == "stripe":
                return self._process_apple_pay_via_stripe(
                    payment_id, amount, currency, payment_token, payment_data
                )
            else:
                # For other gateways, implement similar logic
                return self._process_apple_pay_generic(
                    payment_id,
                    amount,
                    currency,
                    payment_token,
    ...
    def _process_apple_pay_via_stripe(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        payment_method = stripe.PaymentMethod.create(
            type="card",
            card={
                "token": payment_token["paymentData"][
                    "data"
        ...
        rprint(
            f"[green]✅ Apple Pay processed via Stripe (REAL API): ${amount} {currency}[/green]"
        )
        rprint(f"   Payment Intent: {payment_intent.id}")
    ...
    def _process_apple_pay_generic(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        rprint(
            f"[green]✅ Apple Pay token validated (REAL): ${amount} {currency}[/green]"
        )
        rprint(f"   Merchant ID: {merchant_id}")
        rprint(f"   Apple Transaction ID: {payment_token.get('transactionIdentifier')}")
    ...
    def _create_demo_apple_pay_response(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        rprint(
            f"[yellow]⚠️  Apple Pay demo mode (no merchant ID): ${amount} {currency}[/yellow]"
        )
        rprint(f"   Demo Transaction: {demo_transaction_id}")
    ...
    def _process_paypal_payment(
        ...
        if not paypal_client_id or not paypal_client_secret:
            # Fallback to demo mode with clear indication
            rprint(
                f"[yellow]⚠️  PayPal credentials not found - running in demo mode[/yellow]"
            )
            return self._create_demo_paypal_response(
                payment_id, amount, currency, payment_data
            )
        ...
        try:
            ...
            payment = paypalrestsdk.Payment(
                {
                    ...
                    "redirect_urls": {
                        "return_url": payment_data.get(
                            "return_url", "https://chaoschain.com/payment/success"
                        ),
                        "cancel_url": payment_data.get(
                            "cancel_url", "https://chaoschain.com/payment/cancel"
                        ),
                    },
                    "transactions": [
                        {
                            "item_list": {
                                "items": [
                                    {
                                        "name": payment_data.get(
                                            "item_name", "ChaosChain Agent Service"
                                        ),
                            ...
                            "description": payment_data.get(
                                "description", "Payment for ChaosChain agent services"
                            ),
                        }
                    ],
                }
            ...
            if payment.create():
                ...
                rprint(
                    f"[green]✅ PayPal payment created (REAL API): ${amount} {currency}[/green]"
                )
                rprint(f"   Payment ID: {payment.id}")
                rprint(f"   Status: {payment.state}")
                rprint(
                    f"   [blue]User should approve payment at: {approval_url}[/blue]"
                )
            ...
            rprint(
                f"[red]❌ PayPal SDK not installed. Install with: pip install paypalrestsdk[/red]"
            )
            return self._create_demo_paypal_response(
                payment_id, amount, currency, payment_data
            )
    ...
    def _create_demo_paypal_response(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        rprint(
            f"[yellow]⚠️  PayPal demo mode (no credentials): ${amount} {currency}[/yellow]"
        )
    ...
    def _transfer_native_token(
        ...
        to_address: str,
        amount: float,
        token_symbol: str = "A0GI",
    ) -> str:
        """
        Transfer native token (e.g., A0GI on 0G) between addresses.

        Args:
            from_agent: Name of the sending agent
            to_address: Recipient address
            amount: Amount to transfer
            token_symbol: Token symbol (default: A0GI)

        Returns:
            Transaction hash
        """
        rprint(f"[blue]💸 Initiating native {token_symbol} transfer...[/blue]")
        rprint(
            f"   Amount: {amount} {token_symbol} ({self.wallet_manager.w3.to_wei(amount, 'ether')} wei)"
        )
        rprint(f"   From: {from_agent}")
        rprint(f"   To: {to_address}")
        ...
        try:
            ...
            # Get balance
            balance_wei = self.wallet_manager.w3.eth.get_balance(from_address)
            rprint(
                f"   Current balance: {self.wallet_manager.w3.from_wei(balance_wei, 'ether')} {token_symbol}"
            )

            if balance_wei == 0:
                raise PaymentError(f"Insufficient {token_symbol} balance")
            ...
            tx_params = {
                "from": from_address,
                "to": to_address,
                "value": self.wallet_manager.w3.to_wei(
                    amount, "ether"
            ...
            # Estimate gas
            gas_estimate = self.wallet_manager.w3.eth.estimate_gas(tx_params)
            tx_params["gas"] = gas_estimate

            # Build transaction
            tx = self.wallet_manager.w3.eth.account.sign_transaction(
                tx_params, self.wallet_manager.get_private_key(from_agent)
            )
            ...
            receipt = self.wallet_manager.w3.eth.wait_for_transaction_receipt(tx_hash)

            rprint(f"[green]✅ Native transfer successful![/green]")
            rprint(f"   Transaction: {tx_hash.hex()}")

            return tx_hash.hex()

        except Exception as e:
            raise PaymentError(f"Native transfer failed: {str(e)}")
    ...
    def _transfer_usdc(self, from_agent: str, to_address: str, amount: float) -> str:
        ...
        Returns:
            Transaction hash
        """
        # Check if this is a network that uses native token (like 0G)
        if self.network == NetworkConfig.ZEROG_TESTNET:
            return self._transfer_native_token(from_agent, to_address, amount, "A0GI")
        ...
        try:
            ...
            contract = self.wallet_manager.w3.eth.contract(
                address=usdc_address, abi=transfer_abi
            )
            ...
            if balance < amount_wei:
                raise PaymentError(
                    f"Insufficient USDC balance: {balance_usdc} < {amount}"
                )
            ...
            transaction = transfer_function.build_transaction(
                {
                    ...
                    "nonce": self.wallet_manager.w3.eth.get_transaction_count(
                        from_address
                    ),
                }
            )
            ...
            signed_txn = self.wallet_manager.w3.eth.account.sign_transaction(
            ...
            receipt = self.wallet_manager.w3.eth.wait_for_transaction_receipt(
                tx_hash, timeout=120
            )
    ...
    def create_payment_service_flow(
        self, service_type: str, base_amount: float
    ) -> Dict[str, Any]:
        """
